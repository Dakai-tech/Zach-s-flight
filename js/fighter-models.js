...
class FighterModelGenerator {
    ...
    createMissile() {
        ...
        // 导弹主体 - 红色
        const bodyGeom = new THREE.CylinderGeometry(0.08, 0.08, 1.5, 8);
        ...
        // 导弹头部 - 银色尖头
        const noseGeom = new THREE.ConeGeometry(0.08, 0.4, 8);
        ...
        // 尾翼
        const tailWingGeom = new THREE.BoxGeometry(0.3, 0.02, 0.15);
        const tailWing1 = new THREE.Mesh(tailWingGeom, wingMat);
        tailWing1.position.z = -0.6;
        group.add(tailWing1);

        const tailWing2 = new THREE.Mesh(tailWingGeom, wingMat);
        tailWing2.position.z = -0.6;
        tailWing2.rotation.y = Math.PI / 2;
        group.add(tailWing2);

        // 发动机尾焰 - 更亮更大
        const flameGeom = new THREE.SphereGeometry(0.12, 8, 8);
    ...
    createMissileExplosion() {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            const mat = new THREE.MeshBasicMaterial({ 
                color: new THREE.Color().setHSL(0.02 + Math.random() * 0.08, 1, 0.6 + Math.random() * 0.4),
                transparent: true,
                opacity: 0.95
            });
            const particle = new THREE.Mesh(geom, mat);
        ...
        // 冲击波环
        const ringGeom = new THREE.RingGeometry(0.5, 0.8, 16);
        ...
        group.userData = { type: 'missile_explosion', life: 1.0 };
        return group;
    }
    ...
    createPilotBoy() {
        ...
        // 头发 - 黑色短发
        const hairGeom = new THREE.SphereGeometry(0.37, 16, 16, 0, Math.PI * 2, 0, Math.PI * 0.5);
        ...
        const mouthMat = new THREE.MeshBasicMaterial({ color: 0xcc5555 });
        ...
        // 身体 - 飞行员制服（蓝色）
        const bodyGeom = new THREE.CylinderGeometry(0.3, 0.4, 0.9, 8);
        const uniformMat = new THREE.MeshPhongMaterial({ color: 0x2c5aa0 });
        const body = new THREE.Mesh(bodyGeom, uniformMat);
        body.position.y = 0.9;
        group.add(body);

        // 飞行夹克（深绿色）
        const jacketGeom = new THREE.CylinderGeometry(0.32, 0.42, 0.6, 8);
        const jacketMat = new THREE.MeshPhongMaterial({ color: 0x3d5c3d });
        const jacket = new THREE.Mesh(jacketGeom, jacketMat);
        jacket.position.y = 1.0;
        group.add(jacket);

        // 手臂
        const armGeom = new THREE.CylinderGeometry(0.1, 0.08, 0.6, 8);
        const armMat = new THREE.MeshPhongMaterial({ color: 0x3d5c3d });
        ...
        const handMat = new THREE.MeshPhongMaterial({ color: 0xffdbac });

        const leftHand = new THREE.Mesh(handGeom, handMat);
        leftHand.position.set(-0.55, 0.7, 0);
        group.add(leftHand);

        const rightHand = new THREE.Mesh(handGeom, handMat);
        rightHand.position.set(0.55, 0.7, 0);
        group.add(rightHand);

        // 腿
        const legGeom = new THREE.CylinderGeometry(0.12, 0.1, 0.7, 8);
        const legMat = new THREE.MeshPhongMaterial({ color: 0x2c5aa0 });
        ...
        // 飞行头盔
        const helmetGeom = new THREE.SphereGeometry(0.38, 16, 16, 0, Math.PI * 2, 0, Math.PI * 0.6);
        ...
        // 护目镜
        const goggleGeom = new THREE.BoxGeometry(0.5, 0.15, 0.2);
        const goggleMat = new THREE.MeshPhongMaterial({ 
            color: 0x222222,
            transparent: true,
            opacity: 0.8
        });
        const goggle = new THREE.Mesh(goggleGeom, goggleMat);
        goggle.position.set(0, 1.68, 0.25);
        group.add(goggle);

        // 围巾（飘动效果）
        const scarfGeom = new THREE.BoxGeometry(0.5, 0.08, 0.3);
        const scarfMat = new THREE.MeshPhongMaterial({ color: 0xcc3333 });
        const scarf = new THREE.Mesh(scarfGeom, scarfMat);
        scarf.position.set(0, 1.35, -0.15);
        group.add(scarf);

        group.userData = { type: 'pilot_boy' };
        return group;
    }
    ...
    createMapSky() {
        ...
        const skyGeom = new THREE.SphereGeometry(150, 32, 32);
        const skyMat = new THREE.MeshBasicMaterial({ color: 0x87CEEB, side: THREE.BackSide });
        const sky = new THREE.Mesh(skyGeom, skyMat);
        group.add(sky);

        for (let i = 0; i < 25; i++) {
            const cloudGeom = new THREE.SphereGeometry(3 + Math.random() * 4, 8, 8);
            const cloudMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.5 + Math.random() * 0.3 });
            const cloud = new THREE.Mesh(cloudGeom, cloudMat);
            cloud.position.set((Math.random() - 0.5) * 100, -5 + Math.random() * 15, (Math.random() - 0.5) * 100);
            cloud.scale.y = 0.3;
            group.add(cloud);
        }

        for (let i = 0; i < 10; i++) {
            const mountainGeom = new THREE.ConeGeometry(4 + Math.random() * 6, 6 + Math.random() * 10, 6);
            const mountainMat = new THREE.MeshPhongMaterial({ color: 0x3d6b4a });
            const mountain = new THREE.Mesh(mountainGeom, mountainMat);
            mountain.position.set((Math.random() - 0.5) * 120, -10, -30 - Math.random() * 60);
            group.add(mountain);
        }

        const sunGeom = new THREE.SphereGeometry(8, 16, 16);
        const sunMat = new THREE.MeshBasicMaterial({ color: 0xffff88, transparent: true, opacity: 0.8 });
    ...
    createMapStorm() {
        ...
        const skyGeom = new THREE.SphereGeometry(150, 32, 32);
        const skyMat = new THREE.MeshBasicMaterial({ color: 0x2a1a4a, side: THREE.BackSide });
        const sky = new THREE.Mesh(skyGeom, skyMat);
        group.add(sky);

        for (let i = 0; i < 20; i++) {
            const cloudGeom = new THREE.SphereGeometry(4 + Math.random() * 5, 8, 8);
            const cloudMat = new THREE.MeshBasicMaterial({ color: 0x333344, transparent: true, opacity: 0.7 + Math.random() * 0.2 });
            const cloud = new THREE.Mesh(cloudGeom, cloudMat);
            cloud.position.set((Math.random() - 0.5) * 100, 5 + Math.random() * 10, (Math.random() - 0.5) * 100);
            cloud.scale.y = 0.4;
            group.add(cloud);
        }

        for (let i = 0; i < 5; i++) {
            const lightningGeom = new THREE.CylinderGeometry(0.1, 0.1, 15 + Math.random() * 10, 4);
            const lightningMat = new THREE.MeshBasicMaterial({ color: 0xffff00, transparent: true, opacity: 0.6 });
            const lightning = new THREE.Mesh(lightningGeom, lightningMat);
            lightning.position.set((Math.random() - 0.5) * 80, 10, (Math.random() - 0.5) * 80);
            lightning.rotation.z = (Math.random() - 0.5) * 0.5;
            group.add(lightning);
        }

        for (let i = 0; i < 8; i++) {
            const wallGeom = new THREE.BoxGeometry(2, 30, 20);
            const wallMat = new THREE.MeshPhongMaterial({ color: 0x4a3a3a });
            const wall = new THREE.Mesh(wallGeom, wallMat);
            wall.position.set((Math.random() > 0.5 ? 1 : -1) * (40 + Math.random() * 20), 0, (Math.random() - 0.5) * 60);
            group.add(wall);
        }
    ...
    createMapNight() {
        ...
        const skyGeom = new THREE.SphereGeometry(150, 32, 32);
        const skyMat = new THREE.MeshBasicMaterial({ color: 0x0a0a2a, side: THREE.BackSide });
        const sky = new THREE.Mesh(skyGeom, skyMat);
        group.add(sky);

        for (let i = 0; i < 100; i++) {
            const starGeom = new THREE.SphereGeometry(0.1 + Math.random() * 0.2, 4, 4);
            const starMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.5 + Math.random() * 0.5 });
            const star = new THREE.Mesh(starGeom, starMat);
            star.position.set((Math.random() - 0.5) * 150, 20 + Math.random() * 50, (Math.random() - 0.5) * 150);
            group.add(star);
        }

        const moonGeom = new THREE.SphereGeometry(5, 16, 16);
        const moonMat = new THREE.MeshBasicMaterial({ color: 0xffffdd, transparent: true, opacity: 0.9 });
        const moon = new THREE.Mesh(moonGeom, moonMat);
        moon.position.set(-40, 50, -80);
        group.add(moon);

        for (let i = 0; i < 30; i++) {
            const buildingGeom = new THREE.BoxGeometry(3 + Math.random() * 4, 5 + Math.random() * 20, 3 + Math.random() * 4);
            const buildingMat = new THREE.MeshPhongMaterial({ color: 0x1a1a2e, emissive: 0x0a0a1a });
            const building = new THREE.Mesh(buildingGeom, buildingMat);
            building.position.set((Math.random() - 0.5) * 100, -10 + buildingGeom.parameters.height / 2, -20 - Math.random() * 60);
            group.add(building);

            for (let w = 0; w < 5; w++) {
                const windowGeom = new THREE.BoxGeometry(0.3, 0.3, 0.1);
                const windowMat = new THREE.MeshBasicMaterial({ color: Math.random() > 0.5 ? 0xffff88 : 0x8888ff, transparent: true, opacity: 0.8 });
                const window = new THREE.Mesh(windowGeom, windowMat);
                window.position.set(building.position.x + (Math.random() - 0.5) * buildingGeom.parameters.width * 0.8, building.position.y + (Math.random() - 0.5) * buildingGeom.parameters.height * 0.8, building.position.z + buildingGeom.parameters.depth / 2 + 0.1);
                group.add(window);
            }
        }
    ...
    createMapOcean() {
        ...
        const skyGeom = new THREE.SphereGeometry(150, 32, 32);
        const skyMat = new THREE.MeshBasicMaterial({ color: 0x4a6a8a, side: THREE.BackSide });
        const sky = new THREE.Mesh(skyGeom, skyMat);
        group.add(sky);

        const seaGeom = new THREE.PlaneGeometry(200, 200, 20, 20);
        const seaMat = new THREE.MeshPhongMaterial({ color: 0x1a4a6a, shininess: 100, transparent: true, opacity: 0.8 });
        ...
        for (let i = 0; i < 15; i++) {
            const waveGeom = new THREE.CylinderGeometry(2 + Math.random() * 3, 2 + Math.random() * 3, 1, 8);
            const waveMat = new THREE.MeshPhongMaterial({ color: 0x2a5a7a, transparent: true, opacity: 0.6 });
            const wave = new THREE.Mesh(waveGeom, waveMat);
            wave.position.set((Math.random() - 0.5) * 80, -7, (Math.random() - 0.5) * 80);
            group.add(wave);
        }

        for (let i = 0; i < 5; i++) {
            const islandGeom = new THREE.ConeGeometry(5 + Math.random() * 8, 8 + Math.random() * 5, 6);
            const islandMat = new THREE.MeshPhongMaterial({ color: 0x3a5a3a });
            const island = new THREE.Mesh(islandGeom, islandMat);
            island.position.set((Math.random() - 0.5) * 100, -8, -30 - Math.random() * 50);
            group.add(island);
        }

        for (let i = 0; i < 50; i++) {
            const rainGeom = new THREE.CylinderGeometry(0.02, 0.02, 3, 4);
            ...
            rain.position.set((Math.random() - 0.5) * 100, 10 + Math.random() * 20, (Math.random() - 0.5) * 100);
            group.add(rain);
        }
    ...
    createMapVolcano() {
        ...
        const skyGeom = new THREE.SphereGeometry(150, 32, 32);
        const skyMat = new THREE.MeshBasicMaterial({ color: 0x3a1a1a, side: THREE.BackSide });
        const sky = new THREE.Mesh(skyGeom, skyMat);
        group.add(sky);

        for (let i = 0; i < 3; i++) {
            const volcanoGeom = new THREE.ConeGeometry(8 + Math.random() * 5, 15 + Math.random() * 10, 8);
            const volcanoMat = new THREE.MeshPhongMaterial({ color: 0x2a1a1a });
            const volcano = new THREE.Mesh(volcanoGeom, volcanoMat);
            volcano.position.set((Math.random() - 0.5) * 60, -8, -40 - Math.random() * 40);
            group.add(volcano);

            const lavaGeom = new THREE.CircleGeometry(3, 16);
            const lavaMat = new THREE.MeshBasicMaterial({ color: 0xff4400, emissive: 0xff2200, emissiveIntensity: 0.5 });
            const lava = new THREE.Mesh(lavaGeom, lavaMat);
            lava.rotation.x = -Math.PI / 2;
            lava.position.set(volcano.position.x, volcano.position.y + volcanoGeom.parameters.height / 2 - 0.5, volcano.position.z);
            group.add(lava);
        }

        for (let i = 0; i < 30; i++) {
            const particleGeom = new THREE.SphereGeometry(0.2 + Math.random() * 0.3, 6, 6);
            const particleMat = new THREE.MeshBasicMaterial({ color: 0xff6600, emissive: 0xff3300, emissiveIntensity: 0.8, transparent: true, opacity: 0.8 });
            const particle = new THREE.Mesh(particleGeom, particleMat);
            particle.position.set((Math.random() - 0.5) * 80, -5 + Math.random() * 15, (Math.random() - 0.5) * 80);
            group.add(particle);
        }

        for (let i = 0; i < 10; i++) {
            const smokeGeom = new THREE.SphereGeometry(2 + Math.random() * 3, 8, 8);
            const smokeMat = new THREE.MeshBasicMaterial({ color: 0x333333, transparent: true, opacity: 0.4 });
            const smoke = new THREE.Mesh(smokeGeom, smokeMat);
            smoke.position.set((Math.random() - 0.5) * 60, 10 + Math.random() * 20, -30 - Math.random() * 50);
            smoke.scale.y = 2;
            group.add(smoke);
        }

        for (let i = 0; i < 8; i++) {
            const crackGeom = new THREE.BoxGeometry(0.5, 0.1, 10 + Math.random() * 20);
            const crackMat = new THREE.MeshBasicMaterial({ color: 0xff2200, emissive: 0xff1100, emissiveIntensity: 0.5 });
            const crack = new THREE.Mesh(crackGeom, crackMat);
            crack.position.set((Math.random() - 0.5) * 80, -9, (Math.random() - 0.5) * 80);
    ...
    getMap(mapType) {
        switch(mapType) {
            case 'sky': return this.createMapSky();
            case 'storm': return this.createMapStorm();
            case 'night': return this.createMapNight();
            case 'ocean': return this.createMapOcean();
            case 'volcano': return this.createMapVolcano();
            default: return this.createMapSky();
        }
    }

    // 创建BOSS战机（更大的版本）
    createBossFighter(type) {
        const fighter = this.getFighter(type);
        fighter.scale.set(2.5, 2.5, 2.5);

        const ringGeom = new THREE.RingGeometry(4, 5, 32);
        const ringMat = new THREE.MeshBasicMaterial({ color: 0xff0000, transparent: true, opacity: 0.3, side: THREE.DoubleSide });
        const ring = new THREE.Mesh(ringGeom, ringMat);
        ...
        const coreMat = new THREE.MeshBasicMaterial({ color: 0xff3300, transparent: true, opacity: 0.4 });
        const core = new THREE.Mesh(coreGeom, coreMat);
        core.position.y = 1;
        fighter.add(core);

        fighter.userData.isBoss = true;
        return fighter;
    }
    ...
    createBossExplosion() {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            const mat = new THREE.MeshBasicMaterial({ 
                color: new THREE.Color().setHSL(0.0 + Math.random() * 0.1, 1, 0.5 + Math.random() * 0.5),
                transparent: true,
                opacity: 0.95
            });
            const particle = new THREE.Mesh(geom, mat);
            particle.userData = {
                velocity: new THREE.Vector3((Math.random() - 0.5) * 0.8, (Math.random() - 0.5) * 0.8, (Math.random() - 0.5) * 0.8),
                life: 1.0
            };
            group.add(particle);
        }

        const flashGeom = new THREE.SphereGeometry(3.0, 16, 16);
        const flashMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.9 });
        const flash = new THREE.Mesh(flashGeom, flashMat);
        group.add(flash);

        for (let j = 0; j < 3; j++) {
            const ringGeom = new THREE.RingGeometry(1 + j, 1.5 + j, 32);
            const ringMat = new THREE.MeshBasicMaterial({ 
                color: new THREE.Color().setHSL(0.05 * j, 1, 0.6),
                transparent: true,
                opacity: 0.5 - j * 0.1,
                side: THREE.DoubleSide
            });
        ...
        group.userData = { type: 'boss_explosion', life: 1.0 };
        return group;
    }

    // 创建改装视觉效果
    createUpgradedTrail(level) {
        const group = new THREE.Group();
        const count = 5 + level * 2;

        for (let i = 0; i < count; i++) {
            const size = (0.08 + level * 0.02) - i * 0.01;
            const geom = new THREE.SphereGeometry(Math.max(size, 0.02), 6, 6);
            const hue = 0.08 - level * 0.02;
            const mat = new THREE.MeshBasicMaterial({ 
                color: new THREE.Color().setHSL(hue, 1, 0.6 - i * 0.08),
                transparent: true,
                opacity: 0.7 - i * 0.08
            });
            const particle = new THREE.Mesh(geom, mat);
            particle.position.z = -0.3 - i * 0.12;
            group.add(particle);
        }
    ...
    createUpgradedBullet(level) {
        const size = 0.03 + level * 0.01;
        const geom = new THREE.CylinderGeometry(size, size, 0.8 + level * 0.2, 6);
        const hue = 0.15 - level * 0.03;
        const mat = new THREE.MeshBasicMaterial({ 
            color: new THREE.Color().setHSL(hue, 1, 0.5),
            emissive: new THREE.Color().setHSL(hue, 1, 0.3),
            emissiveIntensity: 0.5 + level * 0.2
        });
        const bullet = new THREE.Mesh(geom, mat);
        bullet.rotation.x = Math.PI / 2;
        return bullet;
